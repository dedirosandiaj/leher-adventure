...
export const prisma = {
  admin: {
    ...
    update: async ({ where, data }) => {
      ...
      const sql = `UPDATE Admin SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`;
  ...
  gallery: {
    findMany: async ({ orderBy } = {}) => {
      ...
      if (orderBy?.id) {
        sql += ` ORDER BY id ${orderBy.id === 'desc' ? 'DESC' : 'ASC'}`;
      }
      const result = await db.query(sql);
      return result.rows;
    },
    ...
    create: async ({ data }) => {
      const db = await getDb();
      const result = await db.query(
        'INSERT INTO Gallery (type, title, image, video_url, thumbnail) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [data.type || 'image', data.title, data.image, data.video_url || null, data.thumbnail || null]
      );
      return result.rows[0];
    },
    delete: async ({ where }) => {
  ...
  teamMember: {
    ...
    update: async ({ where, data }) => {
      ...
      values.push(where.id);
      const sql = `UPDATE TeamMember SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`;
      const result = await db.query(sql, values);
      return result.rows[0];
    },
    delete: async ({ where }) => {
